...
const UUID = process.env.UUID || '7939e88d-39fe-49a4-84db-5e333a3578db'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'vertracloud.blibli.kdns.fr';          // 固定隧道域名,留空即启用临时隧道
const DEFAULT_ARGO_AUTH = 'eyJhIjoiOTE5NDQ4ZGZiOWY5MTJiZDhiMGM4MGEyOTgzMTdkYTgiLCJ0IjoiNDlmNzkyYzgtMTM3NC00ZjRhLWI0YzYtZDkyZDJkNGViZWE3IiwicyI6Ik9HWm1OamRpTkdRdE1EZ3lZaTAwWlRrekxUa3hOVGt0T1Rsak1HRm1OMkkzWWpBeCJ9';
const rawArgoAuth = (process.env.ARGO_AUTH || '').trim().replace(/^["']|["']$/g, '');
const ARGO_AUTH = (rawArgoAuth && rawArgoAuth.length > 50) ? rawArgoAuth : DEFAULT_ARGO_AUTH;              // 固定隧道密钥json或token,留空即启用临时隧道
...
const CFIP = process.env.CFIP || ARGO_DOMAIN;               // 节点连接域名或ip，默认直连隧道域名避免第三方saas的403拦截
const CFPORT = process.env.CFPORT || 443;                   // 节点优选域名或优选ip对应的端口
const NAME = process.env.NAME || 'vertracloud';                        // 节点名称
...
async function generateConfig() {
  const config = {
    ...
    inbounds: [
      { tag: 'vless-fallback-in', port: ARGO_PORT, listen: '0.0.0.0', protocol: 'vless', settings: { clients: [{ id: UUID }], decryption: 'none', fallbacks: [{ dest: parseInt(PORT) }, { path: "/vless-argo", dest: 3002 }, { path: "/vmess-argo", dest: 3003 }, { path: "/trojan-argo", dest: 3004 }] }, streamSettings: { network: 'tcp' } },
...
uuid: ${UUID}`;
  ...
  // 运行 Xray
  const webLogFile = path.join(FILE_PATH, 'web.log');
  const command1 = `${webPath} -c ${FILE_PATH}/config.json > "${webLogFile}" 2>&1 &`;
  try {
    await exec(command1);
    ...
    setInterval(() => {
      try {
        if (fs.existsSync(webLogFile)) {
          const size = fs.statSync(webLogFile).size;
          if (size > webLogPos) {
            const fd = fs.openSync(webLogFile, 'r');
            const buf = Buffer.alloc(size - webLogPos);
            fs.readSync(fd, buf, 0, buf.length, webLogPos);
            fs.closeSync(fd);
            webLogPos = size;
            const text = buf.toString('utf8').trim();
            if (text) {
              text.split('\n').forEach(line => {
                if (line.trim()) console.log(`[Xray] ${line.trim()}`);
              });
            }
          }
        }
  ...
  // 网络端口连通性探测 (探测 7844 与 443 端口)
  const net = require('net');
  async function testProbe(host, port) {
    return new Promise((resolve) => {
      const s = net.createConnection({ host, port, timeout: 3500 }, () => {
        s.destroy();
        resolve('CONNECTED (OK)');
      });
      s.on('timeout', () => { s.destroy(); resolve('TIMEOUT (Blocked by firewall)'); });
      s.on('error', (e) => { resolve(`ERROR (${e.code || e.message})`); });
    });
  }

  try {
    const res7844 = await testProbe('198.41.200.233', 7844);
    const res443 = await testProbe('198.41.200.233', 443);
    const resV6 = await testProbe('2606:4700:a0::1', 7844);
    console.log(`[Network Probe] IPv4 198.41.200.233:7844 -> ${res7844}`);
    console.log(`[Network Probe] IPv4 198.41.200.233:443  -> ${res443}`);
    console.log(`[Network Probe] IPv6 [2606:4700:a0::1]:7844 -> ${resV6}`);
  } catch (e) {}
  ...
  if (fs.existsSync(botPath)) {
    let args;
    if (ARGO_AUTH.match(/^[A-Za-z0-9+/=_-]{50,400}$/)) {
      args = `tunnel --protocol http2 --edge-ip-version auto run --token ${ARGO_AUTH}`;
    } else if (ARGO_AUTH.match(/TunnelSecret/)) {
      args = `tunnel --protocol http2 --edge-ip-version auto --config ${FILE_PATH}/tunnel.yml run`;
    } else {
      args = `tunnel --protocol http2 --edge-ip-version auto --logfile ${FILE_PATH}/boot.log --loglevel info --url http://127.0.0.1:${ARGO_PORT}`;
    }
    savedBotArgs = args;

    try {
      const botLogFile = path.join(FILE_PATH, 'bot.log');
      await exec(`${botPath} ${args} > "${botLogFile}" 2>&1 &`);
      console.log(`${botName} is running`);

      // 持续监听 bot.log 并在控制台实时输出
      let botLogPos = 0;
      setInterval(() => {
        try {
          if (fs.existsSync(botLogFile)) {
            const size = fs.statSync(botLogFile).size;
            if (size > botLogPos) {
              const fd = fs.openSync(botLogFile, 'r');
              const buf = Buffer.alloc(size - botLogPos);
              fs.readSync(fd, buf, 0, buf.length, botLogPos);
              fs.closeSync(fd);
              botLogPos = size;
              const text = buf.toString('utf8').trim();
              if (text) {
  ...
  if (ARGO_AUTH && ARGO_DOMAIN) {
    ...
    try {
      ...
      if (argoDomains.length > 0) {
        ...
        const args = `tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile ${FILE_PATH}/boot.log --loglevel info --url http://127.0.0.1:${ARGO_PORT}`;
        try {
...
setInterval(() => {
  try {
    if (fs.existsSync(webPath) && fs.existsSync(configPath)) {
      let isWebRunning = '';
      try {
        isWebRunning = execSync(`pgrep -f "${webPath}" 2>/dev/null || ps aux | grep -v grep | grep "${webPath}" || true`).toString().trim();
      } catch (e) {}
      if (!isWebRunning) {
    ...
    if (fs.existsSync(botPath) && savedBotArgs) {
      let isBotRunning = '';
      try {
        isBotRunning = execSync(`pgrep -f "${botPath}" 2>/dev/null || ps aux | grep -v grep | grep "${botPath}" || true`).toString().trim();
      } catch (e) {}
      if (!isBotRunning) {
        console.log(`[Watchdog] ${botName} exited, restarting...`);
        exec(`${botPath} ${savedBotArgs} > "${path.join(FILE_PATH, 'bot.log')}" 2>&1 &`);
      }
    }
  } catch (err) {}
}, 10000);
